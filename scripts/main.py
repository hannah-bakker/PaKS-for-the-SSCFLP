## -*- coding: utf-8 -*-
# python main.py "../data/i300_1.json" PaKS 3600     
import os
import sys
...
from algs.kernelsearch import KernelSearch
from algs.kernelsearch_configs import KS14, PaKS
...
def _map_config(cfg: str):
    if cfg == "KS14": return dict(KS14)
    if cfg == "PaKS": return dict(PaKS)
    raise ValueError(f"Unknown config '{cfg}'. Expected one of: KS14, PaKS.")
...
def run_instance(path_to_instance: str, config: str, timelimit: int) -> str:
    ...
    if config == "CPX":
        # solve with CPLEX
        model = SSCFLP(inst) 
        cb = model.m.register_callback(CB) # log results in regular intervals
        cb.file = output_path
        cb.data = data

        sopt = model._solve(mipgap = 0.0001, timelimit= timelimit, log_output = True)  #sole regularly with CPLEX 
        data["z-CPLEX"] = sopt.data["obj"]
        data["time-CPLEX"] = sopt.data["time"]  
        data["LB-CPLEX"] = sopt.data["best_bound"]  
        data["MIPGAP-CPLEX"] = sopt.data["mip_rel_gap"] 
        
    else:
        try:
            cfg = _map_config(config)
            cfg["total_timelimit"] = timelimit
            logger.debug("Run KS14 with timelimit=%s",timelimit)
            KS = KernelSearch(instance=inst, problem=SSCFLP, configuration=cfg, data = data, file = output_path, logger = logger)
            logger.info("KernelSearch initialized.")
            KS.run_kernel_search()
            logger.info("KernelSearch finished.")
            KS.get_timings()
            kpis = KS.get_KPIs()
            logger.debug("KPIs collected: %s", {k: kpis.get(k) for k in list(kpis)[:10]})
            data.update(kpis)
        except Exception:
            logger.exception("KernelSearch execution failed.")
            raise
