...
class SSCFLP(MIP):
    ...
    # ------------------------------------------------------------------
    # Single LP relaxation with/without VIs (KS)
    # ------------------------------------------------------------------
    def _get_LP_solution(self, timelimit, logger = None):
        """
        Solve the linear relaxation of the current MIP.

        Parameters
        ----------
        timelimit : int, optional
            Time limit for solving the LP relaxation.
        VIs : str, optional
            How to handle valid inequalities: "all" or "lazy".

        Returns
        -------
        Solution
            Solution object for the LP-relaxation (with reduced costs).
        """
        ...
        if logger: logger.debug("Add all VIs.") 
        self._add_VIs()
        
        if logger: logger.debug(f"Start solving LP-relaxation with timelimit {timelimit}.")
        
        # Build and solve relaxed model
        self.m_LR = LR.make_relaxed_model(self.m) 
        self.m_LR.parameters.timelimit = timelimit
        self.m_LR.context.solver.log_output = True
        
        feasible = self.m_LR.solve()  

        if feasible:
            if logger: logger.info(f"Solved inital LP-relaxation  - z_LP_0: {self.m_LR.solution.get_objective_value()}.")
           
            # Map LR variables into x/y structure for reuse
            ...
            x_vars = sorted(
                dvar_list[I:],
                key=lambda var: (int(var.name.split("_")[1]), int(var.name.split("_")[2])),
            )
            x = np.asarray(x_vars).reshape((I, J))
            ...
            integer_feasible = all(
                int(dvars["y"][i].solution_value) == dvars["y"][i].solution_value
                for i in dvars["y"]
            ) and all(
                int(dvars["x"][ij].solution_value) == dvars["x"][ij].solution_value
                for ij in dvars["x"]
            )
    
            # Reduced costs
            reduced_costs = {}
            reduced_costs_y = self.m_LR.reduced_costs(
                [dvars["y"][i] for i in dvars["y"].keys()]
            )
            reduced_costs["y"] = {i: val for i, val in enumerate(reduced_costs_y)}
            reduced_costs["x"] = {}

            for i in range(I):
                reduced_costs_x_i = self.m_LR.reduced_costs(
                    [x[i, j] for j in range(J)]
                )
        ...
        if logger: logger.debug("Remove all VIs.") 
        self._remove_VIs() 
        return sol
    ...
    def _split_indices(self, 
        ...
        # ------------------------------------------------------------------
        current_subset = 0  # pointer for round-robin over subsets
        # Randomize order in which indices are placed
        for idx in indices:
            placed = False
            tried = 0

            # Try to place idx using round-robin over subsets
            while tried < num_subsets and not placed:
                j = (current_subset + tried) % num_subsets
                if slots_left[j] > 0 and idx not in subsets[j]:
                    subsets[j].add(idx)
                    slots_left[j] -= 1
                    # Next time, continue from the following subset
                    current_subset = (j + 1) % num_subsets
                    placed = True
                else:
                    tried += 1
        ...
        # ------------------------------------------------------------------
        if any(slots_left):
            idx_cycle = itertools.cycle(indices)  # deterministic cycle over indices
            for j in range(num_subsets):
                while slots_left[j] > 0:
                    candidate = next(idx_cycle)
