...
class GetCoordinates:
    ...
    def __init__(self, d_ij, complete_distance_matrix = False,):
        ...
        if self.check_type(d_ij, complete_distance_matrix): #check type
            ...
            result = minimize_scalar(self.bounding_box_area, bounds=(0, np.pi/2), args=(self.coord,), method='bounded')
            optimal_angle = result.x
            
            # Rotate data using the optimal angle
            self.coord = self.rotate_data(self.coord, optimal_angle)
                

 

    def bounding_box_area(self, angle, data):
        """Calculate the bounding box diagonal length after rotation."""
        rotated = self.rotate_data(data, angle)
        min_x, min_y = np.min(rotated, axis=0)
        max_x, max_y = np.max(rotated, axis=0)
        width = max_x - min_x
        height = max_y - min_y
        return width**2 + height**2  # Diagonal squared for optimization

    def rotate_data(self, data, angle):
        """Rotate the data by a given angle (in radians)."""
